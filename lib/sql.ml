...
struct
  ...
  type nullable = Nullable | Strict | Depends [@@deriving eq, show{with_path=false}]

  type t = { t : kind; nullable : nullable; }[@@deriving eq, show{with_path=false}]

  let nullable nullable = fun t -> { t; nullable }
  let strict = nullable Strict
  let depends = nullable Depends
  let nullable = nullable Nullable

  let (=) : t -> t -> bool = equal

  let show { t; nullable} = show_kind t ^ (match nullable with Nullable -> "?" | Depends -> "??" | Strict -> "")
  let _ = pp
  let pp pf t = Format.pp_print_string pf (show t)

  let type_name t = show_kind t.t

  let is_any { t; nullable = _ } = equal_kind t Any
  ...
  let common_nullable = List.fold_left (fun acc t ->
    match acc, t.nullable with
    | _, Nullable
    | Nullable, _ -> Nullable
    | _, Strict
    | Strict, _ -> Strict
  ...
  let common_nullable l = match common_nullable l with Depends -> Strict | n -> n
  let undepend t nullable = if equal_nullable t.nullable Depends then { t with nullable } else t

  let common_type x y =
    match order_nullable x.nullable y.nullable, order_kind x.t y.t with
    | _, `No -> None
    | `Equal nullable, `Order (t,_) -> Some {t; nullable}
    | `Equal _nullable, `Equal -> Some x
