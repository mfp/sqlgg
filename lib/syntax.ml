...
let rec resolve_columns env expr =
  ...
  let rec each e =
    ...
    | Select (select, usage) ->
      ...
      | [ {domain;_} ], `AsValue -> `Func (Type.Ret domain.t, as_params p) (* use nullable *)
...
and assign_types expr =
  ...
  let rec typeof_ (e:expr_q) = (* FIXME simplify *)
    match e with
    | `Value t -> e, `Ok t
    | `Param p -> e, `Ok p.typ
    | `Choice (n,l) ->
    ...
    | `Func (func,params) ->
        ...
        | F (ret, args), _ ->
          ...
          let nullable = common_nullable args in
        ...
        | Ret Any, _ -> (* lame *)
          let (t, types) =
            ...
            | l when List.for_all (function { t = Int | Float; nullable = _ } -> true | _ -> false) l ->
              Float, List.map (fun x -> if is_any x then { x with t = Float } else x) types
            | _ -> Any, types
          and nullable = common_nullable types
          in
          { t; nullable }, types
        | Ret ret, _ ->
          let nullable = common_nullable types in
          { t = ret; nullable; }, types (* ignoring arguments FIXME *)
        in
        let assign inferred x =
          match x with
          | `Param { id; typ; attr } when is_any typ -> `Param (new_param ?attr id inferred)
          | x -> x
        in
        `Func (func,(List.map2 assign inferred_params params)), `Ok ret
  and typeof expr =
    let r = typeof_ expr in
    if !debug then eprintfn "%s is typeof %s" (Type.show @@ get_or_failwith @@ snd r) (show_expr_q @@ fst r);
    r
  in
  typeof expr
